...
def test_put_sandbox_idempotent_same_value_no_save(client):
    ...
    mutate = MagicMock(side_effect=_root_transaction(fake_cfg))
    with (
        patch(
            "qwenpaw.app.routers.config._sandbox_effective_status",
    ...
    # The comparison now runs inside the transaction callback, so the
    # transaction is entered — the proof of "no write" is the test below.
