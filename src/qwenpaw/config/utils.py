...
def mutate_config(
    ...
    Upstream's name for this contract. It delegates so the transactional
    writer above stays the single implementation: the path lock, the
    unknown-field-preserving raw path, and cache invalidation all still
    apply to upstream call sites.
    """
    return update_config_transaction(mutator, config_path)
