...
- `member` 等非特权角色可以聊天、查询数据，以及索要教程、说明、代码片段和配置示例；但不能让 Agent 实际改名、写入记忆、创建/修改/删除文件、修改 QwenPaw 配置、发送外部消息、提交表单或调用写 API。例外：成员可以在聊天页**切换激活模型**（`PUT /models/active` 声明为 CHAT 能力）——注意该操作修改的是全局/智能体级共享配置，对所有用户生效，是有意放开的会话能力。
- 非特权成员可以执行**只读 shell 命令**：命令必须是由白名单查看类命令（`ls`、`cat`、`grep`、`find`、`df`、`ps`、`date`、`wc`、`sort` 等）组成的简单管道；不允许重定向、链式执行（`;`/`&&`/`||`）、子 shell、命令或变量替换（`$`/反引号）、反斜杠转义，参数也不能使用绝对路径、`~` 或 `..`（即只能查看当前 Agent 工作区内相对路径）。`find` 的 `-exec`/`-delete` 等动作旗标同样拒绝。不满足任一条件的命令仍按变更处理并被拒绝。注意：这是执行前的分类放行而非沙箱，白名单命令可读工作区内进程可读的任何文件（包括同一 Agent 其他用户的会话记录）。
...
该列表仅用于诊断，不能启用工具、编辑分类或修改权限。切换所选智能体会重新加载对应智能体的列表；成功保存变更防护设置后，会刷新普通账号的实际允许/拒绝结果。Driver 工具仅覆盖部署级（全局）驱动集合，不包含用户级 Driver 工具。
