...
- Non-privileged roles such as `member` may chat, query data, and request tutorials, explanations, code snippets, or configuration examples. They may not ask the agent to actually rename itself, write memory, create/modify/delete files, change QwenPaw configuration, send external messages, submit forms, or call write APIs. Exception: members may **switch the active model** from the chat page (`PUT /models/active` is declared CHAT) — note this writes shared global/agent-level config and takes effect for all users; it is a deliberate product allowance.
- Non-privileged members may run **read-only shell commands**: the command must be a simple pipeline of whitelisted view-only commands (`ls`, `cat`, `grep`, `find`, `df`, `ps`, `date`, `wc`, `sort`, …). Redirection, chaining (`;`/`&&`/`||`), subshells, command/variable substitution (`$`/backticks), and backslash escapes are rejected, and arguments may not use absolute paths, `~`, or `..` (members stay on relative paths inside the agent workspace). `find` action flags such as `-exec`/`-delete` are rejected too. Anything failing these checks is still treated as a mutation and denied. Note: this is a pre-execution classification, not a sandbox — a whitelisted command can read any workspace file the process can read (including other users' session transcripts of the same agent).
